...
(function($) {
  ...
  $.plone = $.plone || {};
  $.plone.tinymce_instances = {};

  $.plone.tinymce_config = function(id) {
    this.id = id;
    ...
    this.init = function() {
        ...
        if (typeof ($.plone.tinymce_instances[this.id]) != 'undefined') {
            return;
        }
        $.plone.tinymce_instances[this.id] = 1;
  ...
  if (typeof(kukit) != "undefined") {
    ...
    kukit.actionsGlobalRegistry.register("init-tinymce", function(oper) {
      var config = new $.plone.tinymce_config(oper.node.id);
      delete $.plone.tinymce_instances[oper.node.id];
      config.init();
    });
